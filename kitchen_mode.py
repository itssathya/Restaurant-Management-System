...
def orderGet():
    global data
    for line in open("current_orders.txt","r").readlines(): # Read the lines
...
def deleteIt(searchLine):
    with open("current_orders.txt","r") as get:
        lines=get.readlines()
    with open("current_orders.txt","w") as update:
        for line in lines:
...
searchLine=''
data=[["Order Number",'Order Recieve Time',"Order Data"]]
orderGet()
print(tabulate(data))
choice=input("COMPLETE ORDER: ")
if(choice=='b'):
